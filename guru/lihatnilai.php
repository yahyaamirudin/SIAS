    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-sm-6">
          <div class="card">
            <div class="card-body">
              <form method="GET" action="lihatnilai2.php">
                <label>kelas</label>
                <select name="kelas" class="custom-select custom-select mb-3">
                 <?php  
                 $qkelas = mysqli_query($konek,"SELECT DISTINCT Nama_Kelas FROM `nilai_siswa`");

                 while ($row=mysqli_fetch_assoc($qkelas)) {
                  echo '<option value="'.$row['Nama_Kelas'].'">'.$row['Nama_Kelas'].'</option>';}
                  ?>
                </select>
                <select name="semester" class="custom-select custom-select mb-3">
                  <option selected>Semester</option>
                  <?php  
                  $qmapel = mysqli_query($konek,"SELECT DISTINCT Semester FROM `nilai_siswa`");
